...
- **Boot / RAM (jetsam):** Download Screen must **not** load ExecuTorch. Root layout resolves download vs app from **disk readiness and download machine** — in-flight / paused / failed downloads stay on Download Screen and resume; `initExecutorch` runs when entering `(app)` (Continue after download, or cold start with models already present and download settled). Keeps peak RAM under iOS ~2GB ActiveHard during large `.pte` fetches.
